    ...
    class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            try
            {
                ...
                await UpdateHostsFile(sessionConfig.AllowAllPages);
        ...
        private static async Task<SessionConfiguration> GetSessionConfiguration()
        {
            ...
            Console.WriteLine("Host File Configuration:");
            Console.WriteLine("A - All pages (no blocks)");
            Console.WriteLine("L - Limited pages (with blocks)");
            Console.Write("Choose mode (A/L): ");
            
            var modeInput = Console.ReadLine()?.ToUpper();
            while (modeInput != "A" && modeInput != "L")
            {
                Console.Write("Invalid input. Please enter A or L: ");
                modeInput = Console.ReadLine()?.ToUpper();
            }
            
            config.AllowAllPages = modeInput == "A";
        ...
        private static async Task UpdateHostsFile(bool allowAllPages)
        {
            if (!IsRunAsAdministrator())
            {
            ...
            try
            {
                string sourceFile = allowAllPages ? _cleanHostsTemplatePath : _blockedHostsTemplatePath;
                ...
                Console.WriteLine($"Hosts file updated to {(allowAllPages ? "unrestricted" : "blocked")} mode.");
            }
            catch (Exception ex)
            {
